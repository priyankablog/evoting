...
$pag_top +='<div class="input-group-addon">';
$pag_top +='<select onchange="paging(1)" class="nova-perpage" style="background: none;border: none;outline: none;width: 50px;">';
$pag_top +='<option value="10">10</option>';
$pag_top +='<option value="10">20</option>';
$pag_top +='<option value="10">30</option>';
$pag_top +='<option value="25">40</option>';
$pag_top +='<option value="50">50</option>';
$pag_top +='<option value="all">All</option>';
$pag_top +='</select>';
$pag_top +='</div>';
$pag_top +='<input type="text" onkeyup="nsearch(this.value)" id="nova-gsearch" class="form-control" style="margin:0;" placeholder="Search Here ,"/>';
$pag_top +='</div>';
$pag_top +='</div>';
...
function paging(base)
{
    ...
    if($("table").hasClass("nova-pagging"))
    {
        ...
        if($base==1)
        {
            $pbottom+="<li style='color:#555;'>Prev</li>";
        }
        else
        {
            $pbottom+="<li onclick='paging("+($base-1)+")'>Prev</li>";
        }
        ...
        if($base==$ptab)
        {
            $pbottom+="<li style='color:#555;'>Next</li>";
        }
        else
        {
            $pbottom+="<li onclick='paging("+($base+1)+")' >Next</li>";
        }
        $pbottom+="</ul>";
        $pbottom+="<center style='text-align:right;font-size:11px;'>"+($pstart+1)+" to "+$pend+" from "+$totrow+" Records.</center>"; 
    ...
    $mytable.find("tr").each(function(index,row){
        ...
        if(allcell.length>0)
        {
            ...
            allcell.each(function(index,td){
                ...
                {
//                    $orival=$(td).text();
//                     $orival=$orival.replace($vals,"<font color='#d27d08'>"+$vals+"</font>");
//                    $(td).html($orival);
